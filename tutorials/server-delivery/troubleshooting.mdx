...
<Steps>
  <Step title="Confirm the delivery connection">
    For a plugin or script executor, run `gamecart status` from the server or proxy console. For RCON, check the host and port shown in **Integrations > Servers**.
  </Step>
  ...
  <Step title="Follow the command">
    Open **Integrations > Commands** and watch the command status. Click **Refresh** to update the list.
  </Step>
...
<Warning>
  To send a command again, open its row menu and select **Resend**. Resend creates a new delivery attempt and can repeat a command that already ran. Stop any duplicate executor installations and check the server before you click **Resend command**.
</Warning>
