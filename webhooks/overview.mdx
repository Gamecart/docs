...
Return a response quickly. After accepting the event, move slow work to an asynchronous process.
...
<Steps>
  ...
  <Step title="Create an endpoint">
    Click **Create endpoint** and name it after its purpose, such as `Production order worker`.
  </Step>

  <Step title="Enter the destination URL">
    Paste your public HTTPS webhook URL. Gamecart validates it before saving the endpoint or sending events.
  </Step>
...
| `order.payment_approved` | Payment status changes to `APPROVED` from any state other than `DISPUTE`. |
...
| `order.payment_dispute_resolved` | A dispute closes in the store's favor and the payment returns to `APPROVED`. The order was already delivered on the original approval, so this event is an outcome, not a new sale: do not deliver again. |
...
Open an endpoint's **Deliveries** view to inspect recent attempts. Each entry shows the event ID, event type, status, attempt count, latest HTTP status, latest error, and next retry time when applicable.
