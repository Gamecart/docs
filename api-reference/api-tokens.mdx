...
Use **Rotate** if a token may have leaked or your secret policy requires a new value. Gamecart shows the replacement once and disables the previous value.

Use **Revoke** when you retire an integration or no longer trust it. A revoked token cannot authenticate seller API requests.
